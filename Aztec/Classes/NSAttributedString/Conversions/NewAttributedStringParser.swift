...
class NewAttributedStringParser {
    ...
    private struct EdgeData {
        let properties: [ParagraphProperty]
        let styles: [NSAttributedStringKey: Any]
    }
    ...
    func parse(_ attrString: NSAttributedString) -> RootNode {
        var previousRightEdge: EdgeData?
        var nodes = [Node]()

        attrString.enumerateParagraphRanges(spanning: attrString.rangeOfEntireString) { (paragraphRange, enclosingRange) in
            let leftmostAttributes = attrString.attributes(at: 0, effectiveRange: nil)
            let paragraphProperties = self.paragraphProperties(from: leftmostAttributes)
            let inlineStyles = self.inlineStyles(from: leftmostAttributes)
            
            //let leftEdge = EdgeData(properties: properties, styles: )
            
            //let mergeableProperties = mergeable(between: previousProperties, and: properties)
 ...
 */
    ...
    private func inlineStyles(from attributes: [NSAttributedStringKey: Any]) -> [NSAttributedStringKey: Any] {
        return attributes.filter { element -> Bool in
            return element.key != .paragraphStyle
                && !(element.value is NSTextAttachment)
        }
    }
    
    private func mergeable(between previousProperties: [ParagraphProperty], and newProperties: [ParagraphProperty]) -> [ParagraphProperty] {
        var mergeable = [ParagraphProperty]()
        
        for (index, previousProperty) in previousProperties.enumerated() {
            guard newProperties.count > index else {
                break
            }
            
            if previousProperty == newProperties[index] {
                mergeable.append(previousProperty)
            }
        }
